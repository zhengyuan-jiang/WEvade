...
class Decoder(nn.Module):
    ### Receives an image and decodes the watermark. The input image may be watermarked or non-watermarked. Moreover, the input image may have various kinds of noise applied to it, such as JpegCompression, Gaussian blur, and so on. See Noise layers for more.
    ...
    def forward(self, watermarked_image):
        ...
        # The output is of shape b x c x 1 x 1, and we want to squeeze out the last two dummy dimensions and make the tensor of shape b x c. If we just call squeeze_() it will also squeeze the batch dimension when b=1.
